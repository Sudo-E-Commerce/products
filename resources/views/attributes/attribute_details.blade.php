@php
	if (isset($attribute_id) && !empty($attribute_id)) {
		$attribute_details = \Sudo\Product\Models\AttributeDetail::where('attribute_id', $attribute_id)->where('status', '<>', -1)->orderBy('order', 'asc')->orderBy('id', 'asc')->get();
	}
@endphp
<div class="col-lg-9">
	<div class="card listdata" id="listdata">
		...
		<div class="card-body table-responsive p-0">
			<table class="table table-striped table-bordered table-head-fixed">
				...
				<tbody>
					@if (isset($attribute_details) && count($attribute_details) > 0)
						@foreach ($attribute_details as $key => $value)
							<tr data-table="attribute_details" data-id="{!! $value->id !!}">
	...
	<script>
		$(document).ready(function() {
			...
			$('body').on('click', '*[data-delete_attribute]', function(e) {
				e.preventDefault();
				e = $(this);
				// Bảng
